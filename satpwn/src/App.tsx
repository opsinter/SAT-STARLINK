...
import { Satellite, Radio, Loader2, Activity, Wifi } from 'lucide-react';
import { Satellite as SatelliteType, SatellitePass } from './types';
  ...
  useEffect(() => {
    ...
    const fetchStarlinkSatellites = async () => {
      try {
        ...
        const allSatellites = response.data;

        const nearbySatellites = allSatellites
        ...
        setSatellites(nearbySatellites);
        setLastUpdate(new Date());

        const simulatedPasses = nearbySatellites.slice(0, 5).map((sat: SatelliteType) => {
          const now = Date.now();
